...
void fmtx_config_load(uint32_t* hz, FmtxSource* source, FmtxRadio* transmitter) {
    ...
    *transmitter = FmtxRadioAuto;
    ...
    // USB source is disabled on this port (see fmtx_usb.c) -- clamp a
    // config saved before that change, or one carried over from an actual
    // Flipper Zero, back to the only source still offered.
    if(*source != FmtxSourceMp3) *source = FmtxSourceMp3;
}
...
// Upstream ships its two demo MP3s via fap_file_assets, which an on-device
// app manager extracts to APP_ASSETS_PATH (/ext/apps_assets/fmtx) at install
// time -- the same folder pickfile() (fmtx_scenes.c) opens the file browser
// on. This port has no such installer (a .fap is just copied onto the SD
// card directly), so that folder would otherwise never get created, leaving
// users with no obvious place to drop their own MP3s. Create it ourselves.
static void ensure_mp3_folder(void) {
    Storage* storage = furi_record_open(RECORD_STORAGE);
    if(storage) {
        storage_common_mkdir(storage, EXT_PATH("apps_assets"));
        storage_common_mkdir(storage, EXT_PATH("apps_assets/fmtx"));
        furi_record_close(RECORD_STORAGE);
    }
}
