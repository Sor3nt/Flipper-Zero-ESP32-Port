...
// USB source disabled on this port: the low-level USB device/descriptor API
// fmtx_usb.c would need (see its comment) isn't exposed by this port's
// TinyUSB-based HAL, so "Source" below is pinned to a single MP3 entry.
static const char* source_names[] = {"MP3"};
...
static void setin(void* ctx) {
    ...
    item = variable_item_list_add(app->settings_menu, "Source", 1, sourcecb, app);
