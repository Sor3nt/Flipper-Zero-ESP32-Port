...
void tpms_bits_invert(uint8_t* bits, uint16_t nbytes) {
    for(uint16_t i = 0; i < nbytes; i++) bits[i] = (uint8_t)~bits[i];
}
...
uint8_t tpms_xor_bytes(const uint8_t* message, uint16_t bytes) {
    uint8_t result = 0;
    for(uint16_t i = 0; i < bytes; i++) result ^= message[i];
    return result;
}
...
uint32_t tpms_add_nibbles(const uint8_t* message, uint16_t bytes) {
    uint32_t result = 0;
    for(uint16_t i = 0; i < bytes; i++) result += (message[i] >> 4) + (message[i] & 0x0F);
    return result;
}
