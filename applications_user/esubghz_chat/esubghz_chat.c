 ...
 *
 * Adapted for this ESP32 port:
 *  - Dropped bgloader (Unleashed/Momentum-style background-app loader)
 *    support entirely (bgloader_api.h, esubghz_run_with_bgloader(),
 *    esubghz_bgloader_loop(), esubghz_attach_to_gui()/detach_from_gui()) --
 *    that loader mechanism doesn't exist on this port. esubghz_chat() now
 *    always just calls view_dispatcher_run() directly.
 *  - Dropped NFC key-read/share support (nfc_worker/nfc_dev_data alloc+free,
 *    the NfcPopup view) -- see esubghz_chat_i.h for why.
 * Everything else (message TX/RX, encryption, keyboard-lock overlay, chat
 * view hooking) is unchanged from upstream.
 */
#include <furi_hal.h>
...
/* Callback for RX events from the Sub-GHz worker. subghz_tx_rx_worker_*
 * requires a non-NULL callback to be registered, but this port's worker only
 * fires it once per burst (empty -> non-empty transition), which isn't
 * enough to time message completion reliably -- see
 * esubghz_chat_check_messages(), which now polls available() directly
 * instead and doesn't need this callback's timestamp. Kept as a no-op
 * registration target. */
...
static void post_rx(ESubGhzChatState *state, size_t rx_size)
{
	...
	FURI_LOG_I(APPLICATION_NAME, "post_rx: rx_size=%u encrypted=%d", (unsigned)rx_size, state->encrypted);
	for (size_t i = 0; i < rx_size && i < 64; i++) {
		FURI_LOG_I(APPLICATION_NAME, "  byte[%u]=0x%02X ('%c')", (unsigned)i,
				state->rx_buffer[i],
				(state->rx_buffer[i] >= 32 && state->rx_buffer[i] < 127) ? state->rx_buffer[i] : '.');
	}
	...
	FURI_LOG_I(APPLICATION_NAME, "post_rx: final msg='%s' len=%u", state->rx_str_buffer, (unsigned)strlen(state->rx_str_buffer));
...
/* Adapted from upstream: upstream's version re-armed its completion timer
 * from have_read_cb(), which this port's subghz_tx_rx_worker.c only fires
 * once -- on the transition from an EMPTY rx stream buffer to non-empty (see
 * its worker thread: "callback_have_read && bytes_available == 0"). A real
 * chat message from another Flipper typically arrives as several separate
 * over-the-air packets; only the very first one re-arms the timer, so a gap
 * between later packets could make this read out a partial message early
 * (seen on hardware as a message showing up as a blank line). Fixed by
 * polling subghz_tx_rx_worker_available() every tick instead: as long as it
 * keeps growing, more data is still arriving and the timer keeps resetting;
 * only once it holds steady for MESSAGE_COMPLETION_TIMEOUT do we read. */
static void esubghz_chat_check_messages(ESubGhzChatState *state)
{
	size_t avail = subghz_tx_rx_worker_available(state->subghz_worker);
	if (avail == 0) {
		state->rx_last_avail = 0;
		return;
	}

	if (avail != state->rx_last_avail) {
		/* still growing (or just started) -- (re)arm the settle timer */
		state->rx_last_avail = avail;
		state->last_time_rx_data = furi_get_tick();
		return;
	}

	/* avail hasn't changed since the last tick */
...
static bool chat_box_alloc(ESubGhzChatState *state)
{
	...
	/* Dropped: furi_string_set_char(state->chat_box_store, 0, 0) -- upstream
	 * used this to force a null terminator into a freshly-reserved string,
	 * but furi_string_alloc() already returns an empty (size 0) string, so
	 * this was a no-op there anyway. On this port's stricter M*LIB build,
	 * string_set_char() asserts (index) < (string_size(v)); with size still
	 * 0 right after alloc, index 0 fails that check and crashes. */
	text_box_set_text(state->chat_box,
			furi_string_get_cstr(state->chat_box_store));
	text_box_set_focus(state->chat_box, TextBoxFocusEnd);
...
int32_t esubghz_chat(void *args)
{
	UNUSED(args);
	...
	/* add our views to the view dispatcher */
	...
	/* remove our views from the view dispatcher */
