 ...
 * Adapted: sha256() (toolbox/sha256.h, not present on this port) replaced
 * with ESP-IDF's mbedtls_sha256() -- see crypto_wrapper.c for the same swap.
 */
		...
		void *context)
{
	...
	mbedtls_sha256((const unsigned char *) text, strlen(text), key, 0);
