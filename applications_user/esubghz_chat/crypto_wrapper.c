 ...
 * Adapted for this ESP32 port: toolbox/sha256.h doesn't exist here, so the
 * single sha256() call is replaced with ESP-IDF's mbedtls_sha256() (one-shot
 * SHA-256, is224=0). FURI_HAL_CRYPTO_ADVANCED_AVAIL is never defined on this
 * port (no hardware AES-GCM peripheral), so this always takes the bundled
 * software GCM/AES path (crypto/gcm.c, crypto/aes.c -- public domain,
 * unmodified).
 */
#include <furi_hal.h>
#include <m-dict.h>
...
/* Ported code calls the BSD-style __htonl()/__ntohl() names. Those aren't
 * exported symbols on this port (this port's lwip only exposes htonl()/
 * ntohl() macros around a differently-named function), so the FAP loader
 * can't resolve them at runtime. Xtensa is little-endian and this only ever
 * swaps a single 4-byte counter, so a __builtin_bswap32() intrinsic -- not
 * an external symbol, needs no firmware_api.c entry -- is a correct,
 * self-contained drop-in. */
static inline uint32_t __htonl(uint32_t x) {
	return __builtin_bswap32(x);
}
static inline uint32_t __ntohl(uint32_t x) {
	return __builtin_bswap32(x);
}
