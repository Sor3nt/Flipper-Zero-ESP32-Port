...
typedef struct {
    ...
    FuriString* saved_name; // Not part of upstream: base name (no ext) of
        // the .sub file just written, shown on the "Converted" screen.
} Fmf2SubConvertModel;
...
static void fmf2sub_view_convert_draw_callback(Canvas* canvas, void* model) {
    ...
    if(my_model->state == Fmf2SubStateLoading) {
        ...
        // Not part of upstream: show the actual saved filename instead of
        // the generic "Saved in Sub-GHz folder" message.
        canvas_set_font(canvas, FontSecondary);
        if(my_model->saved_name && !furi_string_empty(my_model->saved_name)) {
            char saved_line[40];
            snprintf(
                saved_line,
                sizeof(saved_line),
                "Saved: %s.sub",
                furi_string_get_cstr(my_model->saved_name));
            canvas_draw_str(canvas, 1, 60, saved_line);
        } else {
...
static void fmf2sub_view_convert_exit_callback(void* context) {
    ...
    with_view_model(
        ...
        Fmf2SubConvertModel * model,
        {
            ...
            if(model->saved_name) {
                furi_string_free(model->saved_name);
                model->saved_name = NULL;
            }
        },
        false);
}
...
static uint32_t
    ...
    if(index != FURI_STRING_FAILURE) {
        index += 2;
        size_t size = furi_string_size(song_settings);
        do {
            // Not part of upstream: if the digit run reaches the end of
            // song_settings with no trailing non-digit byte to stop on (e.g.
            // "b=120" as the last, colon-terminated field), the original
            // unconditional furi_string_get_char(index++) reads one byte
            // past the end. Real Flipper firmware's FuriString tolerates
            // that; this port's build asserts on it and crashes -- so treat
            // "ran off the end" the same as "hit a non-digit": stop.
            if(index >= size) {
                break;
            }
...
void fmf2sub_save_sub_file(Fmf2SubApp* app, Fmf2SubConvertModel* model) {
    ...
    // Not part of upstream: name the output .sub after the source .fmf/.txt
    // file instead of the fixed FlipN.sub scheme (upstream's naming is
    // meant for a FlipBoard companion device selecting among several
    // pre-converted signals by button combo -- most users just want the
    // converted file to be identifiable by its source song).
    FuriString* base_name = furi_string_alloc();
    path_extract_filename_no_ext(furi_string_get_cstr(app->file_path), base_name);
    if(furi_string_empty(base_name)) {
        // Fall back to the original FlipN.sub scheme if we somehow don't
        // have a usable source filename (e.g. converting without having
        // loaded a file).
        furi_string_set(base_name, setting_button_values[model->setting_button_index]);
        furi_string_replace_str(base_name, ".sub", "");
    }
    furi_string_cat_printf(file_path, "%s/%s.sub", EXT_PATH("/subghz/"), furi_string_get_cstr(base_name));
    if(model->saved_name == NULL) {
        model->saved_name = furi_string_alloc();
    }
    furi_string_set(model->saved_name, base_name);
    ...
    if(storage_file_open(file, furi_string_get_cstr(file_path), FSAM_WRITE, FSOM_OPEN_ALWAYS)) {
        ...
        // Not part of upstream: the parser below looks ahead past the
        // current character in several places (sharp check, post-note
        // advance, dot check -- each up to furi_string_get_char(notes, i+1)
        // or ++i) without checking against furi_string_size(notes) first.
        // That's fine as long as there's always a trailing byte to land on;
        // real Flipper firmware's FuriString tolerates that one-past-the-end
        // read, but this port's build asserts on it (string_get_char:
        // (index) < (string_size(v))), crashing whenever the last note in
        // the file sits right at the string's end. Pad with a few harmless
        // trailing spaces (skipped by the "ch == ' '" check at the top of
        // the loop) so every lookahead the parser can do stays in bounds.
        furi_string_cat_str(notes, "    ");
        int16_t duration = -1;
        int16_t octave = -1;
        bool dot = false;
...
// Not part of upstream: the file browser (Convert -> OK) opens straight to
// FMF_LOAD_PATH (apps_data/music_player), which real Flipper firmware's
// Music Player app creates on its own. This port has no Music Player app,
// so that folder would otherwise never exist -- create it ourselves so
// there's somewhere to drop .fmf/.txt files via the SD card.
static void ensure_music_folder(void) {
    Storage* storage = furi_record_open(RECORD_STORAGE);
    if(storage) {
        storage_common_mkdir(storage, EXT_PATH("apps_data"));
        storage_common_mkdir(storage, FMF_LOAD_PATH);
        furi_record_close(RECORD_STORAGE);
    }
}
