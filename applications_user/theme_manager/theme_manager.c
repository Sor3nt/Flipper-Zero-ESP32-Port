...
/* Custom animation packs live as subfolders of /ext/dolphin on real
 * hardware (each pack: manifest.txt/meta.txt + frames), alongside
 * dolphin's own top-level manifest.txt. Point the scanner there. */
#ifndef CUSTOM_ANIMATION_PACKS_PATH
#define ANIMATION_PACKS_PATH DOLPHIN_PATH
#else
...
/* The idle-animation picker weighs an applied theme against the flash-baked
 * fallback animations (TV=3, BadBattery=3, NoSd=6, though the latter two get
 * filtered out by name whenever SD/battery are fine). A weight this much
 * higher than TV's 3 means an applied theme wins the vast majority of idle
 * picks (50:3 ≈ 94%) instead of being a 50/50 coin flip against TV. */
#define APPLIED_THEME_WEIGHT 50
...
static bool theme_manager_apply_pack(
    ThemeManagerApp* app,
    const char* merge_src_dir,
...
static void theme_manager_scan_themes(ThemeManagerApp* app) {
    app->theme_count = 0;
    FuriString* backup_manifest_check =
        furi_string_alloc_printf("%s/%s", DOLPHIN_BACKUP_PATH, MANIFEST_FILENAME);
    app->has_backup = storage_file_exists(app->storage, furi_string_get_cstr(backup_manifest_check));
    furi_string_free(backup_manifest_check);
...
// -------------------------------------------------------------------
// ANIMATION_PACKS_PATH is DOLPHIN_PATH itself, so /ext/dolphin/ now doubles
// as both the pack library (browsed by this app) and the live animation
// state. Backing up by moving the whole directory (as this used to do)
// would sweep the entire library into the backup on every apply, leaving
// nothing for the next scan to find. Applying a theme only ever changes
// which folder(s) manifest.txt points at — the pack folders themselves are
// never moved — so it's enough to back up just that one file.
static bool theme_manager_backup_dolphin(ThemeManagerApp* app) {
    if(!storage_file_exists(app->storage, DOLPHIN_MANIFEST)) {
        return true;
    }

    FS_Error mkdir_err = storage_common_mkdir(app->storage, DOLPHIN_BACKUP_PATH);
    if(mkdir_err != FSE_OK && mkdir_err != FSE_EXIST) {
        FURI_LOG_E(TAG, "mkdir failed for dolphin_backup dir (err %d)", mkdir_err);
        return false;
    }

    FuriString* backup_manifest =
        furi_string_alloc_printf("%s/%s", DOLPHIN_BACKUP_PATH, MANIFEST_FILENAME);

    if(storage_file_exists(app->storage, furi_string_get_cstr(backup_manifest))) {
        storage_simply_remove(app->storage, furi_string_get_cstr(backup_manifest));
    }

    FS_Error err = storage_common_rename(
        app->storage, DOLPHIN_MANIFEST, furi_string_get_cstr(backup_manifest));
    furi_string_free(backup_manifest);
    ...
    app->has_backup = true;
    FURI_LOG_I(TAG, "Backed up manifest.txt -> /ext/dolphin_backup/");
    return true;
}
...
//
// A pack's own manifest.txt names each animation folder it ships (this can
// legitimately differ from the pack's own folder/theme name, e.g. for a
// multi-animation pack using weighted random rotation). When the pack
// contains exactly one animation, rename that folder and rewrite the
// manifest so "Name:" matches the theme the user actually applied, so
// /ext/dolphin/manifest.txt reflects what's showing in the picker.
// -------------------------------------------------------------------
static bool theme_manager_apply_pack(
    ThemeManagerApp* app,
    const char* merge_src_dir,
    ...
    FuriString* src_manifest = furi_string_alloc_printf("%s/%s", merge_src_dir, MANIFEST_FILENAME);
    uint32_t anim_count = 0;
    char first_anim_name[MAX_NAME_LEN] = {0};
    bool have_single_name = false;

    if(theme_manager_parse_manifest(app, furi_string_get_cstr(src_manifest), &anim_count) &&
       anim_count == 1) {
        have_single_name = theme_manager_get_first_anim_name(
            app, furi_string_get_cstr(src_manifest), first_anim_name, sizeof(first_anim_name));
    }
    furi_string_free(src_manifest);
    ...
    if(have_single_name && strcmp(first_anim_name, theme_name) != 0) {
        FuriString* old_path = furi_string_alloc_printf("%s/%s", DOLPHIN_PATH, first_anim_name);
        FuriString* new_path = furi_string_alloc_printf("%s/%s", DOLPHIN_PATH, theme_name);
        ...
        if(rename_err == FSE_OK) {
            ...
            if(storage_file_open(manifest, DOLPHIN_MANIFEST, FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
                FuriString* content = furi_string_alloc_printf(
                    ...
                    "Weight: %d\n",
                    theme_name,
                    APPLIED_THEME_WEIGHT);
                const char* str = furi_string_get_cstr(content);
                storage_file_write(manifest, str, strlen(str));
                furi_string_free(content);
                storage_file_close(manifest);
                FURI_LOG_I(TAG, "Renamed %s -> %s, manifest Name matches theme", first_anim_name, theme_name);
            } else {
                FURI_LOG_E(TAG, "Failed to rewrite manifest after rename");
            }
            storage_file_free(manifest);
        } else {
            FURI_LOG_W(
                TAG,
                "Rename %s -> %s failed (err %d), leaving pack's own manifest as-is",
                first_anim_name,
                theme_name,
                rename_err);
        }
...
static bool theme_manager_apply_single(ThemeManagerApp* app, const char* theme_name) {
    ...
    /* ANIMATION_PACKS_PATH is DOLPHIN_PATH itself, so a Single theme found
     * directly under /ext/dolphin/ has src == dst: it's already in place,
     * nothing to copy. */
    bool same_dir = furi_string_equal(src_dir, dst_dir);
    ...
    if(same_dir) {
        ...
        ok = theme_manager_copy_recursive(
...
static bool theme_manager_apply_theme(ThemeManagerApp* app, uint32_t index) {
    ...
    case ThemeTypePack:
        ...
        success = theme_manager_apply_pack(app, furi_string_get_cstr(src), name);
        break;
...
// -------------------------------------------------------------------
// Restore backup: swap the saved manifest.txt back over the active one.
// Pack folders under /ext/dolphin/ are never touched by apply, so nothing
...
static bool theme_manager_restore_backup(ThemeManagerApp* app) {
    ...
    if(storage_file_exists(app->storage, DOLPHIN_MANIFEST)) {
        storage_simply_remove(app->storage, DOLPHIN_MANIFEST);
    }

    FS_Error err = storage_common_rename(
        app->storage, furi_string_get_cstr(backup_manifest), DOLPHIN_MANIFEST);
    furi_string_free(backup_manifest);
    ...
    FURI_LOG_I(TAG, "Restored manifest.txt from /ext/dolphin_backup/");
    return true;
}
...
static void theme_manager_info_draw(Canvas* canvas, void* _model) {
    ...
    canvas_draw_str_aligned(canvas, 64, INFO_TEXT_Y_BTN, AlignCenter, AlignBottom, "[OK]Apply ^Fav");
    canvas_draw_str_aligned(canvas, 126, INFO_TEXT_Y_BTN, AlignRight, AlignBottom, "Del>");
}

// -------------------------------------------------------------------
// Custom Info View — input callback
// Handles Back (left), Delete (right), Apply (OK)
// -------------------------------------------------------------------
static bool theme_manager_info_input(InputEvent* event, void* context) {
    ThemeManagerApp* app = context;
