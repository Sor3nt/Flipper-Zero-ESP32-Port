...
NfcCommand nfcm_nfc_dict_attack_worker_callback(NfcGenericEvent event, void* context) {
    furi_assert(context);
...
void nfcm_nfc_dict_attack_dict_attack_result_callback(DictAttackEvent event, void* context) {
    furi_assert(context);
    NfcMagicApp* instance = context;
...
static void nfc_magic_scene_mf_classic_dict_attack_update_view(NfcMagicApp* instance) {
    ...
    if(mfc_dict->is_key_attack) {
        nfcm_dict_attack_set_key_attack(instance->dict_attack, mfc_dict->key_attack_current_sector);
    } else {
        nfcm_dict_attack_reset_key_attack(instance->dict_attack);
        nfcm_dict_attack_set_sectors_total(instance->dict_attack, mfc_dict->sectors_total);
        nfcm_dict_attack_set_sectors_read(instance->dict_attack, mfc_dict->sectors_read);
        nfcm_dict_attack_set_keys_found(instance->dict_attack, mfc_dict->keys_found);
        nfcm_dict_attack_set_current_dict_key(instance->dict_attack, mfc_dict->dict_keys_current);
        nfcm_dict_attack_set_current_sector(instance->dict_attack, mfc_dict->current_sector);
    }
}
...
static void nfc_magic_scene_mf_classic_dict_attack_prepare_view(NfcMagicApp* instance) {
    ...
    if(state == DictAttackStateUserDictInProgress) {
        do {
            ...
            nfcm_dict_attack_set_header(instance->dict_attack, "MF Classic User Dictionary");
        } while(false);
    }
    if(state == DictAttackStateSystemDictInProgress) {
        ...
        nfcm_dict_attack_set_header(instance->dict_attack, "MF Classic System Dictionary");
    }

    instance->nfc_dict_context.dict_keys_total =
        keys_dict_get_total_keys(instance->nfc_dict_context.dict);
    nfcm_dict_attack_set_total_dict_keys(
        instance->dict_attack, instance->nfc_dict_context.dict_keys_total);
    instance->nfc_dict_context.dict_keys_current = 0;

    nfcm_dict_attack_set_callback(
        instance->dict_attack, nfcm_nfc_dict_attack_dict_attack_result_callback, instance);
...
void nfc_magic_scene_mf_classic_dict_attack_on_enter(void* context) {
    ...
    nfcm_dict_attack_set_card_state(instance->dict_attack, true);
    ...
    nfc_poller_start(instance->poller, nfcm_nfc_dict_attack_worker_callback, instance);
}
...
bool nfc_magic_scene_mf_classic_dict_attack_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == NfcMagicAppCustomEventDictAttackComplete) {
            ...
            nfcm_dict_attack_set_card_state(instance->dict_attack, false);
            consumed = true;
...
void nfc_magic_scene_mf_classic_dict_attack_on_exit(void* context) {
    ...
    nfcm_dict_attack_reset(instance->dict_attack);
    scene_manager_set_scene_state(
        instance->scene_manager,
        NfcMagicSceneMfClassicDictAttack,
        DictAttackStateUserDictInProgress);
