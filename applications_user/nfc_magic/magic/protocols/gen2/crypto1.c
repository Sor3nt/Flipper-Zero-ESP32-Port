...
uint8_t nfcm_crypto1_byte(Crypto1* crypto1, uint8_t in, int is_encrypted) {
    ...
    for(uint8_t i = 0; i < 8; i++) {
        out |= nfcm_crypto1_bit(crypto1, FURI_BIT(in, i), is_encrypted) << i;
    }
    return out;
}
...
uint32_t nfcm_crypto1_word(Crypto1* crypto1, uint32_t in, int is_encrypted) {
    ...
    for(uint8_t i = 0; i < 32; i++) {
        out |= (uint32_t)nfcm_crypto1_bit(crypto1, BEBIT(in, i), is_encrypted) << (24 ^ i);
    }
    return out;
}
...
void nfcm_crypto1_decrypt(Crypto1* crypto, const BitBuffer* buff, BitBuffer* out) {
    ...
    if(bits < 8) {
        ...
        decrypted_byte |= (nfcm_crypto1_bit(crypto, 0, 0) ^ FURI_BIT(encrypted_byte, 0)) << 0;
        decrypted_byte |= (nfcm_crypto1_bit(crypto, 0, 0) ^ FURI_BIT(encrypted_byte, 1)) << 1;
        decrypted_byte |= (nfcm_crypto1_bit(crypto, 0, 0) ^ FURI_BIT(encrypted_byte, 2)) << 2;
        decrypted_byte |= (nfcm_crypto1_bit(crypto, 0, 0) ^ FURI_BIT(encrypted_byte, 3)) << 3;
        bit_buffer_set_byte(out, 0, decrypted_byte);
    } else {
        for(size_t i = 0; i < bits / 8; i++) {
            uint8_t decrypted_byte = nfcm_crypto1_byte(crypto, 0, 0) ^ encrypted_data[i];
...
void nfcm_crypto1_encrypt(Crypto1* crypto, uint8_t* keystream, const BitBuffer* buff, BitBuffer* out) {
    ...
    if(bits < 8) {
        ...
        for(size_t i = 0; i < bits; i++) {
            encrypted_byte |= (nfcm_crypto1_bit(crypto, 0, 0) ^ FURI_BIT(plain_data[0], i)) << i;
        }
        bit_buffer_set_byte(out, 0, encrypted_byte);
    } else {
        for(size_t i = 0; i < bits / 8; i++) {
            uint8_t encrypted_byte = nfcm_crypto1_byte(crypto, keystream ? keystream[i] : 0, 0) ^
                                     plain_data[i];
            bool parity_bit =
...
void nfcm_crypto1_encrypt_reader_nonce(
    ...
    nfcm_crypto1_init(crypto, key);
    if(is_nested) {
        nt_num = nfcm_crypto1_word(crypto, nt_num ^ cuid, 1) ^ nt_num;
    } else {
        nfcm_crypto1_word(crypto, nt_num ^ cuid, 0);
    }

    for(size_t i = 0; i < 4; i++) {
        uint8_t byte = nfcm_crypto1_byte(crypto, nr[i], 0) ^ nr[i];
    ...
    for(size_t i = 4; i < 8; i++) {
        ...
        uint8_t byte = nfcm_crypto1_byte(crypto, 0, 0) ^ (uint8_t)(nt_num);
