...
MfClassicError nfcm_mf_classic_process_error(Iso14443_3aError error) {
    ...
    do {
        ...
        if(is_nested) {
            ...
            if(error != Iso14443_3aErrorNone) {
                ret = nfcm_mf_classic_process_error(error);
                break;
            }
        } else {
            ...
            if(error != Iso14443_3aErrorWrongCrc) {
                ret = (error == Iso14443_3aErrorNone) ? MfClassicErrorProtocol :
                                                        nfcm_mf_classic_process_error(error);
                break;
            }
        }
...
static Gen2PollerError gen2_poller_auth_common(
    ...
    do {
        ...
        nfcm_crypto1_encrypt_reader_nonce(
            instance->crypto,
            key_num,
            cuid,
            nt.data,
        ...
        nfcm_crypto1_word(instance->crypto, 0, 0);
...
Gen2PollerError gen2_poller_halt(Gen2Poller* instance) {
    ...
    do {
        ...
        nfcm_crypto1_reset(instance->crypto);
