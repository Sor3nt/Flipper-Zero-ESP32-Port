    ...
    with_view_model(
        ...
        MifareFuzzerEmulatorModel * model,
        {
            ...
            if(name == NULL) {
                ...
                // Copy the trimmed filename into a persistent buffer instead of
                // stashing a FuriString's internal pointer and leaking the object
                // (this handler runs every attack cycle).
                static char dsc_buf[32];
                ...
                strncpy(dsc_buf, furi_string_get_cstr(card_name), sizeof(dsc_buf) - 1);
                dsc_buf[sizeof(dsc_buf) - 1] = '\0';
                model->mifare_card_dsc = dsc_buf;
