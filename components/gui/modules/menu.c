...
static void menu_draw_callback(Canvas* canvas, void* _model) {
    ...
    if(items_count) {
        ...
        case MenuStylePs4: {
            ...
            char str[10];
            snprintf(str, 10, "%d%%", furi_hal_power_get_pct());
        ...
        case MenuStyleMNTM: {
            ...
            MenuItem* mitem = MenuItemArray_get(model->items, position);
            menu_get_name(mitem, name, true);
            elements_bold_rounded_frame(canvas, 42, 23, 35, 33);
            menu_centered_icon(canvas, mitem, 43, 24, 35, 32);
            ...
            for(size_t i = position; i < endItem; i++) {
                MenuItem* sitem = MenuItemArray_get(model->items, i);
                menu_get_name(sitem, name, true);
