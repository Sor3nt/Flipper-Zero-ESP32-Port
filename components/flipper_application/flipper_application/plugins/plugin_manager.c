 ...
 * @brief Plugin manager for the ESP32 port.
 *
 * Ported from upstream flipperzero-firmware. Loads external .fal plugins via the
 * same flipper_application ELF loader used for FAPs (a plugin is just a FAP with
 * stack_size == 0). Used e.g. by protopirate, which ships its Sub-GHz protocol
 * decoders as fal_embedded plugins under /ext/apps_assets/<appid>/plugins/.
 */
...
// Declared inline (as loader.c does) — the port has no <loader/firmware_api/...> header.
