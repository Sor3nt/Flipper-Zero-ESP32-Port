...
def cmake_quote(path: Path | str) -> str:
    # CMake treats backslashes in quoted strings as escapes; e.g. C:\Users\... becomes invalid \U.
    # Forward slashes work for all paths on Windows in CMake and the toolchains.
    s = str(path).replace("\\", "/")
    return '"' + s.replace('"', '\\"') + '"'
...
def _manifest_path_normalized(manifest_path: str) -> str:
    """Use forward slashes so substring checks work on Windows paths too."""
    return manifest_path.replace("\\", "/")


def generate_js_static_plugins_header(buildset: AppBuildset) -> str:
    """Macros for ESP monolithic JS: only declare/link plugins present in the FAM app set."""
    names = buildset.appnames
    subghz = 1 if "js_subghz" in names else 0
    infrared = 1 if "js_infrared" in names else 0
    return (
        "#pragma once\n"
        "/* Generated by tools/fam/generate.py - do not edit. */\n"
        f"#define FLIPPER_JS_STATIC_HAVE_SUBGHZ {subghz}\n"
        f"#define FLIPPER_JS_STATIC_HAVE_INFRARED {infrared}\n"
    )
...
def generate_ported_cmake(buildset: AppBuildset, project_dir: Path) -> str:
    ...
    for app in sorted_unique_apps(buildset.apps):
        path_n = _manifest_path_normalized(getattr(app, "_manifest_path", ""))
        startup_ok = app.apptype != FlipperAppType.STARTUP or bool(app.sources)
        # Same logic as before: "/a/" in path or ("/applications_user/" in path and startup_ok)
        if "/applications/" in path_n or ("/applications_user/" in path_n and startup_ok):
            ported_apps.append(app)
    ...
    for app in ported_apps:
        ...
        if (
            "/applications_user/" in _manifest_path_normalized(getattr(app, "_manifest_path", ""))
            or app.appid.startswith("js_")
        ):
    ...
    for app in ported_apps:
        ...
        # Flipper / Momentum SD layout uses /ext/<domain>/... (e.g. /ext/subghz/assets/keeloq_mfcodes).
        # App "resources" trees are resources/<domain>/... — merge each top-level child into /ext/<name>/,
        # not under /ext/apps_assets/<appid>/ (which would break EXT_PATH lookups).
        has_apps_assets_copy = False
        for asset_root in asset_roots:
            if asset_root.is_dir() and asset_root.name == "resources":
                for child in sorted(asset_root.iterdir()):
                    ext_dest = f"${{ESP32_FAM_RUNTIME_EXT_ROOT}}/{child.name}"
                    if child.is_dir():
                        stage_commands.append(
                            f"    COMMAND ${{CMAKE_COMMAND}} -E make_directory {cmake_quote(ext_dest)}"
                        )
                        stage_commands.append(
                            f"    COMMAND ${{CMAKE_COMMAND}} -E copy_directory {cmake_quote(child)} {cmake_quote(ext_dest)}"
                        )
                    else:
                        stage_commands.append(
                            f"    COMMAND ${{CMAKE_COMMAND}} -E copy_if_different {cmake_quote(child)} {cmake_quote(ext_dest)}"
                        )
            else:
                has_apps_assets_copy = True

        if has_apps_assets_copy:
    ...
    ext_defaults_root = project_dir / "assets" / "ext_defaults"
    if ext_defaults_root.is_dir():
        for entry in sorted(ext_defaults_root.iterdir()):
            if entry.is_dir():
                destination = f"${{ESP32_FAM_RUNTIME_EXT_ROOT}}/{entry.name}"
                stage_commands.append(
                    f"    COMMAND ${{CMAKE_COMMAND}} -E make_directory {cmake_quote(destination)}"
                )
                stage_commands.append(
                    f"    COMMAND ${{CMAKE_COMMAND}} -E copy_directory {cmake_quote(entry)} {cmake_quote(destination)}"
                )
        stage_depends.update(
            path for path in ext_defaults_root.rglob("*") if path.is_file()
        )
...
def main() -> int:
    ...
    (registry_out.parent / "esp32_js_static_plugins.h").write_text(
        generate_js_static_plugins_header(buildset),
        encoding="utf-8",
    )
